...
# PyTorch 2.6+ compatibility: weights_only default changed from False to True.
# This helper ensures checkpoints load correctly across PyTorch 1.x through 2.11+.
try:
    from deoldify._compat import compatible_torch_load as _torch_load_compat
except ImportError:
...
class Learner():
    ...
    def load(self, file:PathLikeOrBinaryStream=None, device:torch.device=None, strict:bool=True,
        ...
        state = _torch_load_compat(source, map_location=device)
    ...
    def purge(self, clear_opt:bool=True):
        ...
        gc.collect()
        state = _torch_load_compat(tmp_file)
        os.remove(tmp_file)
...
def load_learner(path:PathOrStr, file:PathLikeOrBinaryStream='export.pkl', test:ItemList=None, **db_kwargs):
    ...
    state = _torch_load_compat(source, map_location='cpu') if defaults.device == torch.device('cpu') else _torch_load_compat(source)
