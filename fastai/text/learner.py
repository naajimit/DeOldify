...
class RNNLearner(Learner):
    ...
    def load_encoder(self, name:str, device:torch.device=None):
        ...
        if hasattr(encoder, 'module'): encoder = encoder.module
        encoder.load_state_dict(_torch_load_compat(self.path/self.model_dir/f'{name}.pth', map_location=device))
        self.freeze()
    ...
    def load_pretrained(self, wgts_fname:str, itos_fname:str, strict:bool=True):
        ...
        wgts = _torch_load_compat(wgts_fname, map_location=lambda storage, loc: storage)
